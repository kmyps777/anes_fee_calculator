// ============================================================
//  마취비 계산기 — app.js
//  Firebase compat SDK (window.firebase) 사용
// ============================================================
...
function computeAnesthesiaTime(cases) {
  ...
  return { totalEffectiveMin: timedMin + orphanMin, intervals: merged };
}
...
function calculateFee(cases) {
  ...
  const baseFee = n * 30;
  const freeMin = n * 60;
  const { totalEffectiveMin, intervals } = computeAnesthesiaTime(cases);
  ...
  return { n, baseFee, extraFee, emergTotal, grandTotal, totalEffectiveMin, freeMin, excessMin, extraBlocks, breakdown };
}
...
function getStartTime(c) {
  return c.startH === "" ? "" : `${pad(c.startH)}:${pad(c.startM)}`;
}
...
function loadDraft() {
  try {
    ...
    return (Array.isArray(d.cases) && d.cases.length) ? d : null;
  } catch(e) { return null; }
}
function clearDraft() {
...
(function init() {
  ...
  function renderCases() {
    ...
    cases.forEach((c, idx) => {
          ...
          <div class="duration-group">
            <label class="field-label">마취 시간
              <div class="duration-inputs">
                <input type="number" class="field-input duration-h" min="0" max="23" placeholder="0" value="${durH}" />
                <span class="dur-unit">시간</span>
                <input type="number" class="field-input duration-m" min="0" max="59" placeholder="0" value="${durM}" />
                <span class="dur-unit">분</span>
              </div>
            </label>
          </div>
          <label class="emergency-label">
            <input type="checkbox" class="emergency-check" ${c.emergency ? "checked" : ""} />
      ...
      const hInput = card.querySelector(".duration-h");
      const mInput = card.querySelector(".duration-m");
      const syncDur = () => updateCase(c.id, "durationMin",
        Math.max(0, parseInt(hInput.value)||0) * 60 +
        Math.max(0, Math.min(59, parseInt(mInput.value)||0)));
      hInput.addEventListener("input", syncDur);
      mInput.addEventListener("input", syncDur);

      card.querySelector(".emergency-check").addEventListener("change", e =>
  ...
  async function saveRecord(result) {
    ...
    const col = userCol();
    if (!col) { showToast("Firebase 미연결"); return; }
    try {
      await col.add({
    ...
    try {
      ...
      snap.forEach(docSnap => {
        ...
        card.querySelector(".btn-del-rec").addEventListener("click", async e => {
          e.stopPropagation();
          try { await docSnap.ref.delete(); showToast("기록 삭제됨"); loadHistory(); }
  ...
  if (auth) {
    ...
    auth.getRedirectResult()
      .then(r => { if (r && r.user) showToast("✓ 로그인 완료"); })
